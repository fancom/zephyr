...
static inline bool is_event_queue_full(void) {
	...
	uint16_t next = head + 1;
    	if (next == CONFIG_NET_MGMT_EVENT_QUEUE_SIZE) {
		next = 0;
   	}
...
static inline bool is_event_queue_empty(void) {
	// We define empty as head == tail */
    	return (head == tail);
}

static inline void advance_event_queue_pointer(void) {
	if(is_event_queue_full()) {
		if (++tail == CONFIG_NET_MGMT_EVENT_QUEUE_SIZE) {
			tail = 0;
		}
	}

	if (++head == CONFIG_NET_MGMT_EVENT_QUEUE_SIZE) {
		head = 0;
	}
}

static inline void retreat_event_queue_pointer(void)
{
	/* We need to handle the wraparound case */
	if (++tail == CONFIG_NET_MGMT_EVENT_QUEUE_SIZE) {
		tail = 0;
				   ...
				   const void *info, size_t length)
{
	...
	events[head].iface = iface;
    	...
    	advance_event_queue_pointer();


#ifdef CONFIG_NET_MGMT_EVENT_INFO
	if (info && length) {
		if (length <= NET_EVENT_INFO_MAX_SIZE) {
			memcpy(events[head].info, info, length);
			events[head].info_length = length;
		} else {
...
static void mgmt_thread(void)
{
	...
	while (1) {
		...
		if (!mgmt_event) {
			 ...
			 * more events than we could handle
			 */
			NET_WARN("Some event got probably lost (%u)",
				k_sem_count_get(&network_event));
