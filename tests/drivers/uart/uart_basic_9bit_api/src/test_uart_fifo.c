...
static const uint16_t fifo_data[] = {
...
#define DATA_SIZE	(sizeof(fifo_data)/sizeof(uint16_t))
...
static void uart_fifo_callback(const struct device *dev, void *user_data)
{
	...
	if (uart_irq_tx_ready(dev) && tx_data_idx < DATA_SIZE) {
		 ...
		 */
		if (uart_fifo_fill9(dev,
				   (uint16_t *)&fifo_data[tx_data_idx++], 1) > 0) {
	...
	if (uart_irq_rx_ready(dev)) {
		...
		uart_fifo_read9(dev, &recvData, 1);
		TC_PRINT("0x%03X", recvData);
